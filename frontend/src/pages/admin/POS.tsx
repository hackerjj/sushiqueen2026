...
interface POSCartItem extends CartItem {
  line_total: number;
  person: number; // which guest ordered this
}
...
const POS: React.FC = () => {
  ...
  const fetchMenu = useCallback(async () => {
    try {
      ...
      setItems(list.filter(i => i.available).length > 0 ? list.filter(i => i.available) : menuData);
  ...
  const fetchTables = async () => {
    try {
      ...
      const list = Array.isArray(data.data) ? data.data : [];
      setTables(list.length > 0 ? list : defaultTables);
    } catch { setTables(defaultTables); }
  };
  ...
  const searchCustomers = async (phone: string) => {
    ...
    try { const { data } = await api.get(`/admin/customers?search=${phone}`); setCustomerMatches(Array.isArray(data.data) ? data.data.slice(0, 5) : []); setShowMatches(true); } catch { setCustomerMatches([]); }
  };
  ...
  const addToCart = (item: MenuItem) => {
    setCart(prev => {
      const existing = prev.find(c => c.menu_item_id === item._id && c.person === 1);
      if (existing) return prev.map(c => c.menu_item_id === item._id && c.person === 1 ? { ...c, quantity: c.quantity + 1, line_total: (c.quantity + 1) * c.price } : c);
      return [...prev, { menu_item_id: item._id, name: item.name, quantity: 1, price: item.price, modifiers: [], line_total: item.price, person: 1 }];
    });
  };

  const updateQty = (id: string, person: number, delta: number) => {
    setCart(prev => prev.map(c => c.menu_item_id === id && c.person === person ? { ...c, quantity: c.quantity + delta, line_total: (c.quantity + delta) * c.price } : c).filter(c => c.quantity > 0));
  };

  const changePerson = (id: string, oldPerson: number, newPerson: number) => {
    setCart(prev => prev.map(c => c.menu_item_id === id && c.person === oldPerson ? { ...c, person: newPerson } : c));
  };
  ...
  const sendToKitchen = async () => {
    ...
    try {
      ...
      await api.post('/orders', {
        customer: { name: customerName || `Mesa ${selectedTableNum}`, phone: customerPhone || '', address: customerAddress },
        items: cart.map(c => ({ menu_item_id: c.menu_item_id, name: c.name, price: c.price, quantity: c.quantity, modifiers: [], person: c.person })),
        source: 'pos', type: orderType, payment_method: paymentMethod,
  ...
  if (showTicket) return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white max-w-sm w-full shadow-xl rounded-xl">
        <div ref={ticketRef} className="p-6 font-mono text-sm" style={{ maxWidth: '300px', margin: '0 auto' }}>
          ...
          <table className="w-full text-xs">
            <thead><tr className="border-b border-gray-300"><th className="text-left py-1">Cant</th><th className="text-left py-1">Producto</th><th className="text-right py-1">Precio</th></tr></thead>
            <tbody>
              {cart.map((c, i) => (
                <tr key={i} className="border-b border-gray-100">
                  <td className="py-1">{c.quantity}</td>
                  <td className="py-1">{c.name}{orderType === 'dine_in' ? ` (P${c.person})` : ''}</td>
                  <td className="py-1 text-right">{fmt(c.line_total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <p className="text-xs mt-1">Pago: {paymentMethod === 'cash' ? 'Efectivo' : paymentMethod === 'card' ? 'Tarjeta' : 'Transferencia'}</p>
        ...
        <div className="p-4 border-t flex gap-2 print:hidden">
          ...
          <button onClick={closeOrder} className="flex-1 py-2 rounded-lg bg-green-500 text-white text-sm">Cerrar orden</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="h-screen bg-gray-100 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-2 overflow-x-auto">
          ...
          {categories.map(c => (
            <button key={c} onClick={() => setCategory(c)} className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${category === c ? 'bg-sushi-primary text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{c}</button>
          ))}
        </div>
          ...
          {orderType === 'dine_in' && !selectedTable ? (
            <div>
              <div className="flex gap-2 mb-4">
                {zones.map(z => (<button key={z} onClick={() => setActiveZone(z)} className={`px-4 py-2 rounded-lg text-sm font-medium ${activeZone === z ? 'bg-white shadow text-gray-900 border' : 'bg-gray-200 text-gray-600'}`}>{z}</button>))}
              </div>
      ...
      <div className="w-80 bg-white border-l border-gray-200 flex flex-col">
        ...
        <div className="px-4 py-2 border-b border-gray-100 flex gap-2">
          {(['dine_in', 'takeout', 'delivery'] as OrderType[]).map(t => (
            <button key={t} onClick={() => { setOrderType(t); if (t !== 'dine_in') { setSelectedTable(''); setSelectedTableNum(0); } }} className={`flex-1 py-1.5 rounded-lg text-xs font-medium ${orderType === t ? 'bg-sushi-primary text-white' : 'bg-gray-100 text-gray-600'}`}>
        ...
        <div className="px-4 py-2 border-b border-gray-100 space-y-2">
          {orderType === 'dine_in' && selectedTable && (
            <div className="flex items-center gap-2">
              ...
              <select value={guestCount} onChange={(e) => setGuestCount(Number(e.target.value))} className="flex-1 border border-gray-300 rounded-lg px-2 py-1 text-xs">
          ...
          {orderType === 'delivery' && (
            <div className="relative">
              <input value={customerPhone} onChange={(e) => { setCustomerPhone(e.target.value); searchCustomers(e.target.value); }} placeholder="Teléfono *" className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-xs" />
              {showMatches && customerMatches.length > 0 && (<div className="absolute z-10 w-full bg-white border rounded-lg shadow-lg mt-1 max-h-32 overflow-y-auto">{customerMatches.map(c => (<button key={c._id} onClick={() => selectCustomer(c)} className="w-full text-left px-3 py-2 hover:bg-gray-50 text-xs border-b last:border-0"><span className="font-medium">{c.name}</span> — {c.phone}</button>))}</div>)}
              <input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Nombre *" className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-xs mt-2" />
              <input value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} placeholder="Dirección *" className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-xs mt-2" />
            </div>
          )}
        </div>
        {/* Cart items with person selector */}
        <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2">
          {cart.length === 0 ? (
            ...
            <div key={`${item.menu_item_id}-${item.person}-${idx}`} className="bg-gray-50 rounded-lg px-3 py-2">
              ...
              {orderType === 'dine_in' && guestCount > 1 && (
                <select value={item.person} onChange={(e) => changePerson(item.menu_item_id, item.person, Number(e.target.value))} className="mt-1 text-xs border border-gray-200 rounded px-2 py-0.5 bg-white text-gray-600">
                  {Array.from({ length: guestCount }, (_, i) => <option key={i + 1} value={i + 1}>Persona {i + 1}</option>)}
                </select>
              )}
            </div>
          ))}
